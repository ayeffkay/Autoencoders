...
class TrainUtility:
    ...
    def gen_out(self, batch: dict, epoch_out: dict):
        for key in epoch_out.keys():
            try:
              epoch_out[key] = torch.cat((epoch_out[key], batch[key]), dim=0)
            except:
              epoch_out[key] = batch[key]
        return epoch_out

    def run_epoch(self, data, mode='valid', return_out=False):
        self.epoch_loss = 0
        epoch_out = dict.fromkeys(['feature', 'code', 'label'])
        ...
        with torch.set_grad_enabled(mode=='train'):
            ...
            for i, batch in enumerate(data):
                ...
                if return_out:
                    batch['code'] = out.latent_code
                    epoch_out = self.gen_out(batch, epoch_out)
    ...
    def extract_out(self, dl_name='valid'):
        out = self.run_epoch(self.dataloaders[dl_name], mode='test', return_out=True)
        return out


    def get_2d_embeddings(self, dl_name='valid', show=True):
        out = self.extract_out(dl_name)
        codes, label = out['code'].cpu().numpy(), out['label'].cpu().numpy()

        tsne = TSNE(n_components=2)
        embeddings = tsne.fit_transform(codes)
        if show:
            self.plot_2d_embeddings(embeddings, label)
    ...
    def plot_2d_embeddings(embeddings, labels):
        ...
        x, y = np.hsplit(embeddings, 2)
        plt.scatter(x, y, c=labels, cmap='Set1', alpha=0.5)
    ...
    def get_similar(self, img, n_neighbors=5, show=True):
        ...
        dist, ind = list(map(lambda x: x.squeeze(0), [dist, ind]))

        if show:
            dist = list(map(lambda d: 'Dist.={:.3f}'.format(d), dist))
            data_utils.plot_gallery(torch.cat([img, out['feature'][ind]], dim=0), 
                                    n_row=1, n_col=n_neighbors + 1, 
                                    subplot_titles = ['Original image'] + dist)
        return dist, out['feature'][ind]
